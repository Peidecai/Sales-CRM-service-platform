  ...
  <view class="check-in-page">
    ...
    <view class="location-card">
      ...
      <view v-else-if="currentLocation" class="location-info">
        ...
        <text v-if="currentLocation.accuracy" class="location-accuracy">
    ...
    <!-- Nearby Customers -->
    <view v-if="nearbyCustomers.length > 0" class="form-section">
      <view class="form-label">附近客户推荐</view>
      <view
        v-for="nc in nearbyCustomers"
        :key="nc.customer.id"
        class="nearby-item"
        @click="selectCustomer(nc.customer)"
      >
        <view class="nearby-info">
          <text class="nearby-name">{{ nc.customer.name }}</text>
          <text class="nearby-addr">{{ nc.customer.address || '无地址' }}</text>
        </view>
        <text class="nearby-dist">{{ formatDist(nc.distance) }}</text>
      </view>
    </view>
...
import { checkInApi, type CheckInVO, type CheckInStats } from '@/api/check-in'
import { haversineDistance, formatDistance } from '@/utils/geo'
import { getHighAccuracyLocation, reverseGeocode, type LocationResult } from '@/native/geo'
import { offlineQueue } from '@/utils/offline-queue'

const CHECK_IN_RADIUS = 500 // meters

interface NearbyCustomer {
  customer: CustomerVO & { lat?: number; lng?: number }
  distance: number
}
...
const nearbyCustomers = ref<NearbyCustomer[]>([])
...
async function refreshLocation() {
  ...
  try {
    ...
    if (result) {
      ...
      const address = result.address || await reverseGeocode(result.latitude, result.longitude)
...
function updateNearbyCustomers() {
  if (!currentLocation.value || searchResults.value.length === 0) {
    nearbyCustomers.value = []
    return
  }

  const loc = currentLocation.value
  const withDistance: NearbyCustomer[] = []

  for (const c of searchResults.value) {
    const cust = c as CustomerVO & { lat?: number; lng?: number }
    if (cust.lat && cust.lng) {
      const dist = haversineDistance(loc.latitude, loc.longitude, cust.lat, cust.lng)
      withDistance.push({ customer: cust, distance: dist })
    }
  }

  // Sort by distance, take top 5
  withDistance.sort((a, b) => a.distance - b.distance)
  nearbyCustomers.value = withDistance.slice(0, 5)
}
...
function selectCustomer(customer: CustomerVO) {
  selectedCustomer.value = customer as CustomerVO & { lat?: number; lng?: number }
...
async function handleSubmit() {
  ...
  try {
    ...
    offlineQueue.push('POST', '/check-in', data as unknown as Record<string, unknown>)
