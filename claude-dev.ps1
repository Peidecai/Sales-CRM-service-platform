# Claude Code Automated Development Script
# Usage:
#   .\claude-dev.ps1
#   .\claude-dev.ps1 -StartFrom 3
#   .\claude-dev.ps1 -SaveLogs -Verify
#   .\claude-dev.ps1 -Help
...
param(
    ...
    [switch]$Verify,
    [string]$VerifyCmd = "pnpm lint && pnpm build",
    [switch]$Force,
    [switch]$ShowCmd,
    [switch]$Help
)
...
if ($Help) {
    Write-Host ""
    Write-Host "Claude Code Automated Development Script" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  .\claude-dev.ps1 [-TaskFile path] [-Model name] [-MaxTurns N] [-StartFrom N]"
    ...
    Write-Host "  -Model       Claude model ID         (default: claude-opus-4-6)"
    ...
    Write-Host "  -Verify      Run verify command after all tasks"
    Write-Host "  -VerifyCmd   Verify command string   (default: pnpm lint && pnpm build)"
    Write-Host "  -Force       Allow running on main/master branch"
    Write-Host "  -ShowCmd     Print the claude command + git diffs"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  .\claude-dev.ps1"
    Write-Host "  .\claude-dev.ps1 -StartFrom 5 -SaveLogs"
    Write-Host "  .\claude-dev.ps1 -Verify -VerifyCmd 'pnpm test'"
    Write-Host "  .\claude-dev.ps1 -MaxTurns 80 -PauseSecs 5 -SaveLogs -Verify"
    Write-Host ""
    exit 0
}
...
# Parse tasks: blank-line separated blocks, # = comment
...
if ($SaveLogs) {
    ...
    $sessionHeader = "Session: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')`nModel: $Model`nTasks: $($tasks.Count)`nStartFrom: $StartFrom`n"
...
for ($i = 0; $i -lt $tasks.Count; $i++) {
    ...
    if ($isGitRepo -and $ShowCmd) {
        ...
        Write-Log "Git status before: $($gitBefore.Count) changed file(s)" "DEBUG"
    }
    ...
    try {
        ...
        # Tee-Object: stream each line to console in real-time AND capture into $outputLines
        ...
        $prompt | & claude `
            ...
            --max-turns $MaxTurns `
            2>&1 | ForEach-Object {
                Write-Host $_          # real-time display
                $outputLines.Add($_)   # capture for log
            }
        ...
        Write-Host "=== End (${elapsed}s) ===" -ForegroundColor DarkCyan
        Write-Host ""
        ...
        if ($isGitRepo) {
            ...
            if ($changed) {
                Write-Log "Files changed: $($changed.Count) file(s)" "INFO"
                if ($ShowCmd) {
