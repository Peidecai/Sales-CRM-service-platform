...
"@
    ...
    "P0-05" = @{
        ...
        Prompt = @"
修复双前缀路由 bug。main.ts 已通过 setGlobalPrefix('api/v1') 设置全局前缀，以下 Controller 不应再包含 'api/v1'：
...
"@
    ...
    "P0-06" = @{
        ...
        Prompt = @"
三个 Controller 都使用 @Controller('call') 前缀，路由可能冲突。按职责分离为不同前缀：
...
"@
    ...
    "P1-05" = @{
        ...
        Prompt = @"
在 packages/server/src/common/filters/http-exception.filter.ts 中：
...
"@
    ...
    "P1-06" = @{
        ...
        Prompt = @"
在 packages/server/src/config/jwt.config.ts 中：
...
"@
    ...
    "P1-07" = @{
        ...
        Prompt = @"
在 packages/server/src/common/redis/cache-keys.ts 中：
...
"@
    ...
    "P1-09" = @{
        ...
        Prompt = @"
在 packages/server/src/main.ts 中完善 Swagger 配置：
...
"@
    ...
    "P1-10" = @{
        ...
        Prompt = @"
在 packages/server/src/common/logger/ 目录下：
...
"@
    ...
    "P1-11" = @{
        ...
        Prompt = @"
在 packages/server/src/config/ 目录下：
...
"@
    ...
    "P1-12" = @{
        ...
        Prompt = @"
在 packages/server/src/modules/health/ 目录下：
...
"@
    ...
    "P1-13" = @{
        ...
        Prompt = @"
在 packages/server/src/common/dto/pagination.dto.ts 中：
...
"@
    ...
    "P1-14" = @{
        ...
        Prompt = @"
在 packages/server/src/common/interceptors/ 目录下：
...
"@
    ...
    "P1-16" = @{
        ...
        Prompt = @"
在 packages/server/src/common/pipes/ 目录下：
...
"@
    ...
    "P1-17" = @{
        ...
        Prompt = @"
在 packages/server/src/common/middleware/ 目录下：
...
"@
    ...
    "P2-02" = @{
        ...
        Prompt = @"
在 packages/server/src/common/entities/base.entity.ts 中：
...
"@
    ...
    "P3-04" = @{
        ...
        Prompt = @"
将 getWrapUpOverdueAgentIds() 中的 KEYS 命令改为 SCAN：
...
function Get-TasksToRun {
    ...
    if ($TaskIds.Count -gt 0) {
        ...
        foreach ($id in $TaskIds) {
            if ($Script:Tasks.ContainsKey($id)) {
                $tasks += @{ Id = $id; Info = $Script:Tasks[$id] }
            }
            else {
        ...
        # 按优先级过滤
        foreach ($key in $Script:Tasks.Keys | Sort-Object) {
...
"@
    ...
    $outputFile = Join-Path $OutputDir "fix-$TaskId-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
...
function Show-TaskList {
    ...
    $grouped = $Script:Tasks.GetEnumerator() | Group-Object { $_.Value.Priority }
    
    foreach ($group in $grouped | Sort-Object Name) {
        Write-Log "`n[$($group.Name)] 优先级任务:"
        foreach ($item in $group.Group | Sort-Object Name) {
            $status = if ($item.Value.Files.Count -gt 0) { "[$($item.Value.Files.Count) 文件]" } else { "" }
            Write-Log "  $($item.Name) | $($item.Value.Category) | $($item.Value.Title) $status"
        }
    }
    
    Write-Log "`n总计: $($Script:Tasks.Count) 个任务"
    Write-Log "P0: $(($Script:Tasks.Values | Where-Object { $_.Priority -eq 'P0' }).Count)"
    Write-Log "P1: $(($Script:Tasks.Values | Where-Object { $_.Priority -eq 'P1' }).Count)"
    Write-Log "P2: $(($Script:Tasks.Values | Where-Object { $_.Priority -eq 'P2' }).Count)"
    Write-Log "P3: $(($Script:Tasks.Values | Where-Object { $_.Priority -eq 'P3' }).Count)"
}

# 生成报告
function Generate-Report {
    $reportFile = Join-Path $OutputDir "fix-report-$(Get-Date -Format 'yyyyMMdd-HHmmss').md"
...
function Main {
    ...
    foreach ($task in $tasks) {
        $i++
        Write-Log "`n[$i/$($tasks.Count)] 开始执行: $($task.Id)"
